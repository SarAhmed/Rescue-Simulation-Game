	...
	private void loadUnits(String filePath) throws IOException {
		...
		for (String s : unitsArray) {
			...
			case "DCU":
				...
				DiseaseControlUnit diseaseControlUnit = new DiseaseControlUnit(input[1], world[0][0],
			...
			case "EVC":
				...
				Evacuator evacuator = new Evacuator(input[1], world[0][0], Integer.parseInt(input[2]), this,
			...
			case "GCU":
				...
				GasControlUnit gasControlUnit = new GasControlUnit(input[1], world[0][0], Integer.parseInt(input[2]),
						this);
	...
	public void assignAddress(Simulatable sim, int x, int y) {
		if (sim instanceof Citizen) {
			...
			if (unit.getLocation().getX() != x && unit.getLocation().getY() != y) {
				...
				CommandCenter.getView().getGrid().removeUnit(unit, unit.getLocation().getX(),
		...
		for (int i = 0; i < plannedDisasters.size(); i++) {
			...
			if (disaster.getStartCycle() == currentCycle) {
				...
				if (disaster instanceof Fire) {
					if (disaster.getTarget().getDisaster() != null
							&& disaster.getTarget().getDisaster() instanceof GasLeak) {
						...
						if (building.getGasLevel() == 0) {
							...
							try {
								toBeStrike.strike();
								logString += "~ Disaster " + toBeStrike.getDisasterType() + " hit "
										+ toBeStrike.getTarget().targetType() + "at location :"
						...
						try {
							disaster.strike();
							logString += "~ Disaster " + disaster.getDisasterType() + " hit "
									+ disaster.getTarget().targetType() + " at location :"
					...
					if (disaster.getTarget().getDisaster() != null
							&& disaster.getTarget().getDisaster() instanceof Fire) {
						...
						try {
							collapse.strike();
							logString += "~ Disaster " + collapse.getDisasterType() + " hit "
									+ collapse.getTarget().targetType() + " at location :"
...
//----------------------
		...
		for (int i = 0; i < executedDisasters.size(); i++) {
			if (executedDisasters.get(i).isActive()) {
				logString += executedDisasters.get(i).getDisasterType() + " is affecting "
						+ executedDisasters.get(i).getTarget().targetType() + " at location "
