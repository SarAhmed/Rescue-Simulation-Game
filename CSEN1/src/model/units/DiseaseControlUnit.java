	...
	public void respond(Rescuable r) throws IncompatibleTargetException, CannotTreatException {
		if (!(r instanceof Citizen)) {
			throw new IncompatibleTargetException(this, r,
					"You can't send a disease control unit to treat a building !");
		}
		if (!canTreat(r)) {
	...
	public void treat() {
		...
		if (citizen.getHp() == 0) {
			...
			if (disaster instanceof Infection) {
				...
				if (citizen.getToxicity() == 0) // have changed with comparison with solution (but we already handled it
												// in setToxicity but this in case if we changed setToxicity)
					citizen.setState(CitizenState.RESCUED);
