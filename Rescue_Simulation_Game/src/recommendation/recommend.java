		...
		for (ResidentialBuilding building : gasleakBuildings) {
			...
			if (best != null) {
				...
				if (best.getTarget() != building) {
					...
					recommend += "Dispatch Gas Control Unit " + best.getUnitID() + " to the building in location : x="
							+ building.getLocation().getX() + " y=" + building.getLocation().getY();
					recommend += "\n";
				}
			}
		}
		...
		for (ResidentialBuilding building : fireBuildings) {
			...
			if (best != null) {
				...
				if (best.getTarget() != building) {
					...
					recommend += "Dispatch FireTruck " + best.getUnitID() + " to the building in location : x="
							+ building.getLocation().getX() + " y=" + building.getLocation().getY();
					recommend += "\n";
				}
			}
	...
	public static String recommendEvacuator() {
		...
		for (ResidentialBuilding building : collapseBuildings) {
			...
			for (Unit unit : Evacuator) {
				...
				t += (building.getLocation().getX() + building.getLocation().getY()) * 2.0 / e.getStepsPerCycle()
						* e.getMaxCapacity();
				if (t > size) {
					size = t;
					best = unit;
			...
			if (best != null) {
				...
				if (best.getTarget() != building) {
					recommend += "Dispatch Evacuator " + best.getUnitID() + " to the building in location : x="
							+ building.getLocation().getX() + " y=" + building.getLocation().getY();
					recommend += "\n";
				}
			}
	...
	public static String recommendAmbulance() {
		...
		for (Citizen c : injuriedCitizen) {
			...
			if (best != null) {
				...
				if (best.getTarget() != c) {
					recommend += "Dispatch Ambulance " + best.getUnitID() + " to the citizin in location : x="
							+ c.getLocation().getX() + " y=" + c.getLocation().getY();
					recommend += "\n";
				}
			}
		}
	...
	public static String recommendDiseaseControl() {
		...
		for (Citizen c : infectedCitizen) {
			...
			if (best != null) {
				...
				if (best.getTarget() != c) {
					recommend += "Dispatch DiseaseControlUnit " + best.getUnitID() + " to the citizin in location : x="
							+ c.getLocation().getX() + " y=" + c.getLocation().getY();
					recommend += "\n";
				}
			}
