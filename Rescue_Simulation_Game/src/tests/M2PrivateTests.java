...
public class M2PrivateTests {
	...
	DiseaseControlUnit testDiseaseContorlUnit = new DiseaseControlUnit(id4, testAddress1, 1, null);
	GasControlUnit testGasContorlUnit = new GasControlUnit("5", testAddress1, 1, null);
	...
	// HELPERS
	private boolean checkValueLogic(Object createdObject, String name, Object value) throws Exception {
		Field f = null;
	...
	private void clearArrayList(Simulator s) throws Exception {
		final Field unitField = Simulator.class.getDeclaredField("emergencyUnits");
		unitField.setAccessible(true);
		((ArrayList<Object>) unitField.get(s)).clear();
	}

	private void applyDisaster(Disaster d, ResidentialBuilding b) throws Exception {
		final Field disasterField = ResidentialBuilding.class.getDeclaredField("disaster");
	...
	public void testConstructorDiseaseControlUnit() throws Exception {
		Class[] inputs = { String.class, Address.class, int.class, WorldListener.class };
	...
	public void testConstructorEvacuatorUnit() throws Exception {
		Class[] inputs = { String.class, Address.class, int.class, WorldListener.class, int.class };
		testConstructorExists(Class.forName(evacuatorPath), inputs);
	}

	@Test(timeout = 3000)
	public void testAmbulanceTreat() throws Exception {
		testExistsInClass(Class.forName(ambulancePath), "treat", true, void.class);
	}

	@Test(timeout = 3000)
	public void testDiseaseControlUnitTreat() throws Exception {
		testExistsInClass(Class.forName(diseaseControlUnitPath), "treat", true, void.class);
	}
	...
	public void testUnitRespondFromIdleLogic() throws Exception {
		...
		u.respond(c);
		assertEquals("Unit respond should initialize the target correctly.", c, u.getTarget());
		assertTrue("Unit respond should update the distanceToTarget correctly.",
				checkValueLogic(u, "distanceToTarget", 3));
		assertEquals("Unit respond should update the unitState correctly.", UnitState.RESPONDING, u.getState());
	}
	...
	public void testDiseaseControlUnitTreatLogic() throws Exception {
		...
		for (int i = 0; i < 5; i++) {
			u.treat();
			assertTrue("Values of toxicity were wrong, expected " + toxicityValues.get(i) + " but was "
					+ c1.getToxicity() + ".", toxicityValues.get(i) == c1.getToxicity());
		}
		...
		assertTrue("Ambulance should deactivate the disaster upon starting the treatment process",
				checkValueLogic(d, "active", false));
		assertTrue("Ambulance should mark the citizen as RESCUED after treating their injury.",
	...
	public void testAmbulanceCallsHeal() throws Exception {
		...
		assertTrue("The method cycleStep should call the method heal if the unit arrives to valid target", healCalled);
		healCalled = false;
	}
	...
	public void testUnitCycleStepIdleLogic() throws Exception {
		...
		assertTrue("cycleStep should not change the unit location if it is idle.", checkValueLogic(u1, "location", ad));
		assertTrue("cycleStep should not change the unit target if it is idle.", checkValueLogic(u1, "target", null));
		assertTrue("cycleStep should not change the unit distanceToTarget if it is idle.",
	...
	public void testGasControlUnitTreatLogic() throws Exception {
		...
		u.cycleStep();
		assertTrue("GasControlUnit should not change the disaster, it should activate it or deactivate it only.",
				checkValueLogic(c1, "disaster", d));
		assertTrue("GasControlUnit treat should deactivate the disaster.", checkValueLogic(d, "active", false));
		assertTrue("GasControlUnit unit state should be changed to IDLE once completing the treatment.",
	...
	public void testEvacuatorTreatLogicCycle0() throws Exception {
		...
		Evacuator u = new Evacuator("evacuator1", getAddressFromWorld(s, 3, 4), 5, null, 2);
		u.setWorldListener(s);
		...
		assertEquals("Unit location should be updated upon arrival to target location.", ad, u.getLocation());
		assertEquals("Unit state should remain responding until 1 cycle after arrival.", UnitState.RESPONDING,
				u.getState());

		assertEquals("Evacuator should start evacuating citizen the next cycle after arrival.", 0,
				u.getPassengers().size());
		assertEquals("Number of citizens remaining in the building should not be changed till evacuation starts.", 5,
				b.getOccupants().size());

		assertEquals("first Citizen should be in the location " + ad.getX() + " " + ad.getY() + " but was "
				+ citizensToBeTested.get(0).getLocation().getX() + " " + citizensToBeTested.get(0).getLocation().getY(),
				ad, citizensToBeTested.get(0).getLocation());

		assertEquals("second Citizen should be in the location " + ad.getX() + " " + ad.getY() + " but was "
				+ citizensToBeTested.get(1).getLocation().getX() + " " + citizensToBeTested.get(1).getLocation().getY(),
				ad, citizensToBeTested.get(1).getLocation());

		assertEquals("third Citizen should be in the location " + ad.getX() + " " + ad.getY() + " but was "
				+ citizensToBeTested.get(2).getLocation().getX() + " " + citizensToBeTested.get(2).getLocation().getY(),
				ad, citizensToBeTested.get(2).getLocation());

		assertEquals("fourth Citizen should be in the location " + ad.getX() + " " + ad.getY() + " but was "
				+ citizensToBeTested.get(3).getLocation().getX() + " " + citizensToBeTested.get(3).getLocation().getY(),
				ad, citizensToBeTested.get(3).getLocation());

		assertEquals("fifth Citizen should be in the location " + ad.getX() + " " + ad.getY() + " but was "
				+ citizensToBeTested.get(4).getLocation().getX() + " " + citizensToBeTested.get(4).getLocation().getY(),
				ad, citizensToBeTested.get(4).getLocation());
	...
	public void testEvacuatorTreatLogicCycle1() throws Exception {
		...
		assertEquals("Evacuator distanceToBase should be updated before returning to Base.", 12, u.getDistanceToBase());
		assertEquals("Unit location should not be updated while en route.", ad, u.getLocation());
		assertEquals("Unit state should be updated to Treating when the unit is treating.", UnitState.TREATING,
				u.getState());
		assertEquals("Evacuator should start evacuating citizen the next cycle after arrival.", 2,
				u.getPassengers().size());
		assertEquals("Number of citizens remaining in the building should not be changed till evacuation starts.", 3,
				b.getOccupants().size());
	...
	public void testEvacuatorTreatLogicCycle2() throws Exception {
		...
		assertEquals("Evacuator distanceToBase should be updated while returning to Base.", 7, u.getDistanceToBase());

		assertEquals("Unit location should not be updated while en route.", ad, u.getLocation());
		assertEquals("Unit state should not be updated as long as the unit is treating.", UnitState.TREATING,
				u.getState());
		assertEquals("Evacuator should not drop the citizens until it reaches the base.", 2, u.getPassengers().size());
	...
	public void testEvacuatorTreatLogicCycle4() throws Exception {
		...
		if (!checkValueLogic(u, "distanceToTarget", 12))
			u.cycleStep();
		assertEquals("Evacuator distanceToBase should be updated while returning to Base.", 0, u.getDistanceToBase());
		assertTrue("Unit distanceToTarget should be updated after each cycle.",
				checkValueLogic(u, "distanceToTarget", 12));

		assertEquals("Unit location should not be updated while en route.", safe, u.getLocation());
		assertEquals("Unit state should be not be updated as long as the unit is treating.", UnitState.TREATING,
				u.getState());
		assertEquals("Evacuator should drop citizens upon reaching the base.", 0, u.getPassengers().size());

		assertEquals("Number of citizens remaining in the building should not be changed till evacuation restarts.", 3,
				b.getOccupants().size());

		assertEquals("first Citizen should be in the location " + safe.getX() + " " + safe.getY() + " but was "
				+ citizensToBeTested.get(0).getLocation().getX() + " " + citizensToBeTested.get(0).getLocation().getY(),
				safe, citizensToBeTested.get(0).getLocation());
		assertEquals("first Citizen state should be rescued but was " + citizensToBeTested.get(0).getState(),
				CitizenState.RESCUED, citizensToBeTested.get(0).getState());

		assertEquals("second Citizen should be in the location " + safe.getX() + " " + safe.getY() + " but was "
				+ citizensToBeTested.get(1).getLocation().getX() + " " + citizensToBeTested.get(1).getLocation().getY(),
				safe, citizensToBeTested.get(1).getLocation());

		assertEquals("second Citizen state should be rescued but was " + citizensToBeTested.get(1).getState(),
				CitizenState.RESCUED, citizensToBeTested.get(1).getState());
	...
	public void testEvacuatorTreatLogicCycle8() throws Exception {
		...
		if (u.getDistanceToBase() != 12)
			u.cycleStep();
		assertEquals("Evacuator distanceToBase should be updated while returning to Base.", 12, u.getDistanceToBase());
		assertTrue("Unit distanceToTarget should be updated after each cycle.",
				checkValueLogic(u, "distanceToTarget", 0));
		...
		assertEquals("Unit state should be not be updated as long as the unit is treating.", UnitState.TREATING,
				u.getState());
		assertEquals("Evacuator should drop citizens upon reaching the base.", 2, u.getPassengers().size());

		assertEquals("Number of citizens remaining in the building should not be changed till evacuation restarts.", 1,
				b.getOccupants().size());
	...
	public void testStrikeInfection() throws Exception {
		...
		infection2.strike();
		assertTrue("Method \"strike\" in class \"Infection\" should set the initial \"toxicity\" to 25",
	...
	public void testStrikeCollapse() throws Exception {
		...
		collapse2.strike();
		assertTrue("Method \"strike\" in class \"Collapse\" should set the initial \"foundationDamage\" to 10",
	...
	public void testCycleStepInjury() throws Exception {
		...
		for (int i = 1; i <= 5; i++) {
			injury2.cycleStep();
			assertTrue("Method \"cycleStep\" in class \"Injury\" should set the \"bloodLoss\" to " + i * 10
					+ " by cycle " + i + " but was " + citizen0.getBloodLoss(), citizen0.getBloodLoss() == i * 10);
		}
	}
	...
	public void testCycleStepFire() throws Exception {
		...
		for (int i = 1; i <= 5; i++) {
			fire2.cycleStep();
			assertTrue(
					"Method \"cycleStep\" in class \"Fire\" should set the \"fireDamage\" to " + i * 10 + " by cycle "
							+ i + " but was " + residentialBuilding2.getFireDamage(),
	...
	public void testCompoundDisaster3() throws Exception {
		...
		final Field disasterField = Simulator.class.getDeclaredField("plannedDisasters");
		disasterField.setAccessible(true);

		((ArrayList<Object>) disasterField.get(s)).clear();
		((ArrayList<Object>) disasterField.get(s)).add(testFire);
		final Field currentcyclefield = Simulator.class.getDeclaredField("currentCycle");
		currentcyclefield.setAccessible(true);
		currentcyclefield.set(s, 2);
		final Field buildingField = Simulator.class.getDeclaredField("buildings");
		...
		final Field e_disasterField = Simulator.class.getDeclaredField("executedDisasters");
		e_disasterField.setAccessible(true);
		((ArrayList<Object>) e_disasterField.get(s)).add(testGasLeak);
		clearArrayList(s);

		s.nextCycle();
		assertEquals("The Disaster should be removed from planned disasters arraylist after striking ", 0,
				((ArrayList<Object>) disasterField.get(s)).size());
		ResidentialBuilding b = (ResidentialBuilding) ((ArrayList<Object>) buildingField.get(s)).get(0);
		...
		assertEquals(temp + "The Structural Integrity value of the building should be 0 ", 0,
	...
	public void testGameOverCase4() throws Exception {
		...
		final Field disasterField = Simulator.class.getDeclaredField("plannedDisasters");
		disasterField.setAccessible(true);
		final Field executeDisasterField = Simulator.class.getDeclaredField("executedDisasters");
		...
		final Field unitsField = Simulator.class.getDeclaredField("emergencyUnits");
		...
		assertTrue("check gameOver in class simulator should return True when there are units that aren't IDLE",
				s.checkGameOver());
	}
	...
	public void testSimulatorInitializeCitizenListener() throws Exception {
		...
		ArrayList<Citizen> testCitizen = (ArrayList<Citizen>) citizenField.get(s);
		final Field listenerField = Citizen.class.getDeclaredField("emergencyService");
		listenerField.setAccessible(true);

		for (int i = 0; i < testCitizen.size(); i++) {
			boolean cond = testCitizen.get(i).getWorldListener() != null;
			assertTrue("Wotldlistener value in class Citizen should be instantiated after calling simulator", cond);

			cond = listenerField.get(testCitizen.get(i)) != null;
			assertTrue("SOS Listener value in class Citizen should be instantiated after calling simulator", cond);
		}
	}
	...
	public void testCitizenInstanceVariableEmergencyListener() throws Exception {
		...
		testGetterMethodExistsInClass(Citizen.class, "getWorldListener", WorldListener.class, true);
		testSetterMethodExistsInClass(Citizen.class, "setWorldListener", WorldListener.class, true);
		testSetterLogic(someRandomCitizen(), "worldListener", new Simulator(new CommandCenter()), WorldListener.class);
	}
	...
	public void testCitizenStruckByUpdatesState() throws Exception {
		...
		try {
			...
			Field emergencyService = Citizen.class.getDeclaredField("emergencyService");
			emergencyService.setAccessible(true);
			emergencyService.set(ct, cc);

			Method struckBy = Citizen.class.getDeclaredMethod("struckBy", Disaster.class);
	...
	public void testCitizenToxicitySetterBounds() throws Exception {
		...
		try {
			...
			assertEquals("The toxicity value of any Citizen can not be set to more than 100.", 100, toxicityValue);
			...
			assertEquals("The toxicity value of any Citizen can not be set to less than 0.", 0, toxicityValue);
	...
	public void testResidentialBuildingGasLevelSetterGreaterOrEqualOneHundredKillsAllOccupants()
		...
		try {
			...
			b.setGasLevel(r);
			ArrayList<Citizen> occupantsAfter = (ArrayList<Citizen>) occupants.get(b);
			for (int i = 0; i < occupantsAfter.size(); i++) {
				...
				try {
					...
					assertEquals(
							"if the gas level of a building is set to 100 or above, all of its occupants should die", 0,
							hpValue);
				} catch (NoSuchFieldException e) {
	...
	@Test(timeout = 1000)
	public void testResidentialBuildingCycleStep2() throws IllegalArgumentException, IllegalAccessException {
		Address d = someRandomAddress();
		ResidentialBuilding b = new ResidentialBuilding(d);
		try {
			Field structuralIntegerity = ResidentialBuilding.class.getDeclaredField("structuralIntegrity");
	...
	@Test(timeout = 1000)
	public void testResidentialBuildingCycleStep3() throws IllegalArgumentException, IllegalAccessException {
		Address d = someRandomAddress();
		ResidentialBuilding b = new ResidentialBuilding(d);
		...
		try {
			foundationDamage = ResidentialBuilding.class.getDeclaredField("foundationDamage");
		} catch (NoSuchFieldException e) {
		...
		try {
			structuralIntegrity = ResidentialBuilding.class.getDeclaredField("structuralIntegrity");
	...
	@Test(timeout = 1000)
	public void testResidentialBuildingCycleStep4() throws IllegalArgumentException, IllegalAccessException {
		Address d = someRandomAddress();
		ResidentialBuilding b = new ResidentialBuilding(d);
		Field fireDamage = null;
		Field structuralIntegrity = null;
		try {
			fireDamage = ResidentialBuilding.class.getDeclaredField("fireDamage");
		} catch (NoSuchFieldException e) {
	...
	@Test(timeout = 1000)
	public void testResidentialBuildingCycleStep5() throws IllegalArgumentException, IllegalAccessException {
		Address d = someRandomAddress();
		ResidentialBuilding b = new ResidentialBuilding(d);
		Field fireDamage = null;
		Field structuralIntegrity = null;
