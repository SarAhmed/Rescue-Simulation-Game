...
public class ResidentialBuilding implements Rescuable, Simulatable, Comparable<ResidentialBuilding> {
	...
	public void struckBy(Disaster d) {
		if (disaster != null) // have changed with comparison with the solution (but it's redundant we handled
								// it in nextCycle)
	...
	public boolean allDead() {
		...
		for (int i = 0; i < occupants.size(); i++) {
			if (occupants.get(i).getState() != CitizenState.DECEASED)
				return false;
