		...
		if (target != null) {
			if ((!(this instanceof MedicalUnit)) && (getState() == UnitState.TREATING)) {
				...
				if (citizen.getBloodLoss() != 0 || citizen.getToxicity() != 0) {
					if (getState() == UnitState.TREATING)// have changed with comparison with the solution( but it's
															// redundant because if the state not treating so it will be
															// IDle and the target will be null or responding and the
