...
public class Evacuator extends PoliceUnit {
	...
	public void respond(Rescuable r) throws IncompatibleTargetException, CannotTreatException {
		...
		if (!canTreat(r)) {
			...
			throw new CannotTreatException(this, r,
					"The evacuator can't handle " + r.getDisaster().getDisasterType() + " !");
	...
	public void cycleStep() {
		if (getState() != UnitState.IDLE && getTarget() != null) {
			if (this.getDistanceToBase() > 0) {
				...
				if ((!this.getPassengers().isEmpty()) && this.getLocation().getX() == 0
					...
					for (Citizen c : this.getPassengers()) {
						if (c.getState() != CitizenState.SAFE) {
							...
							cell.setIcon(null);
							cell.setIcon(new ImageIcon(CommandCenter.getView().getGrid().getCommandCentreUnitImg()
					...
					setDistanceToTarget(calculateDistance(this.getTarget().getLocation())); // have changed with
																							// comparison with the
																							// solution to pass the
																							// private tests (we did it
																							// in the previous cycle-
																							// it's commented up here-
																							// but it doesn't matter i
																							// dont know why it makes
																							// problem)
	...
	public void treat() {
		// super.treat();
		this.setState(UnitState.TREATING);// have changed with comparison with solution and according to the description
											// since collapse shouldn't be deactivated
		System.out.println("BAARA");
		ResidentialBuilding residentialBuilding = (ResidentialBuilding) this.getTarget();
		if (residentialBuilding.getLocation().getX() == this.getLocation().getX()
				&& residentialBuilding.getLocation().getY() == this.getLocation().getY() && getPassengers().isEmpty()) { // i
																															// am
																															// in
																															// the
																															// target
			...
			{
				if (this.getPassengers().size() < getMaxCapacity()
						&& residentialBuilding.getStructuralIntegrity() != 0) {
		...
		if (residentialBuilding.allDead() && residentialBuilding.getLocation().getX() == this.getLocation().getX()
				&& residentialBuilding.getLocation().getY() == this.getLocation().getY() && getPassengers().isEmpty()) {
			jobsDone();
